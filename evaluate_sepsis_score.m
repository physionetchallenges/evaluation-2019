...
function evaluate_sepsis_score(label_directory, prediction_directory, output_file)
    ...
    if length(label_files) ~= length(prediction_files)
        error('Numbers of label and prediction files must be the same.');
    end
    ...
    for k = 1 : num_files
        ...
        if ~(length(labels) == length(predictions) || length(predictions) == length(probabilities))
            error('Numbers of labels and predictions for a file must be the same.');
        end
        ...
        for i = 1 : num_records
            ...
            if ~(probabilities(i) >= 0 || probabilities(i) <= 1)
                disp('Probabilities do not satisfy 0 <= probability <= 1.');
            end
        end
        ...
        if min_probability_positive <= max_probability_negative
            disp('Predictions are inconsistent with probabilities, i.e., a positive prediction has a lower (or equal) probability than a negative prediction.');
        end
...
function column = load_column(filename, header)
    ...
    for i = 1 : size(data, 2)
        if strcmp(column_names(i), header)
            column = data(:, i);
            break;
        end
    end
...
function [auroc, auprc] = compute_auc(labels, predictions)
    ...
    for i = 1 : n
        if ~(predictions(i) >= 0 && predictions(i) <= 1)
            disp('Predictions do not satisfy 0 <= prediction <= 1.');
        end
    end
...
function [accuracy, f_measure] = compute_accuracy_f_measure(labels, predictions)
    ...
    for i = 1 : n
        if ~(predictions(i) == 0 || predictions(i) == 1)
            disp('Predictions do not satisfy prediction == 0 or prediction == 1.');
        end
    end
